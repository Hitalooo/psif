...
class SimulacaoJurosSimples(Simulacao):
    ...
    def simular(self) -> tuple[float, float, float]:
        '''Simula o rendimento baseado em juros simples.
        ...
        # Cálculo do montante acumulado usando a fórmula M = C x (1 + i x t)
        if self.taxa_juros > 0:
            montante_acumulado = mensalidade_total * self.periodo_meses * (1 + self.taxa_juros)
        else:
            montante_acumulado = mensalidade_total * self.periodo_meses  # Sem juros, apenas somando
